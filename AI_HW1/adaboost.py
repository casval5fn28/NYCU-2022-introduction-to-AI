...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
                First , set curr_th(current threshold) as "0" and curr_pl(current polarity)
            as "1" , then initialize "bestClf" by "WeakClassifier" with parameters I just 
            set and set "bestError" as infinity .
                Then read the array of HaarFeature class . During the process , first 
            set currClf(current Clf) and currError(current error) , but initialize 
            currError as 0 . Also in HaarFeature class reading process , set variable
            "tmp" and traverse the array of "label" , if currClf.polarity * featureVals 
            " < " currClf.polarity * currClf.threshold , then make "tmp" as "1" (if not , 
            make it as "0") . Current Error keeps iterating with corresponding weights and 
            absolute value of ( corresponding label - "tmp" ) , leading to the final error.
                Finally , if currError " < "  bestError , then replace bestError with the 
            smaller one , so we can get a result with lower error rate .
                
        """
        curr_th = 0
        curr_pl = 1
        bestClf = WeakClassifier(features[0], threshold = curr_th, polarity = curr_pl)
        bestError = math.inf
        
        for i in range(len(features)):
            currClf = WeakClassifier(features[i], threshold = curr_th, polarity = curr_pl)
            currError = 0
            tmp = 0
            for j in range(len(labels)):
                sample_val = featureVals[i,j]
                if currClf.polarity * sample_val < currClf.polarity * currClf.threshold:
                    tmp = 1
                else:
                    tmp = 0
                    
                currError += ( weights[j] * abs(labels[j] - tmp) )
