...
class Agent():
    ...
    def learn(self):
        ...
        q_value = self.evaluate_net(state)
        next_q_value = self.target_net(next_state)


        batch_idx = np.arange(self.batch_size, dtype=np.int64)
        curr_val = q_value[batch_idx, action]
        fur_val = torch.max(next_q_value, dim=1)[0]
        done = 1 - done#
        q_target = reward + self.gamma * fur_val * done        
        tmp = nn.MSELoss()
        loss = tmp(q_target, curr_val)
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            if random.uniform(0,1) < self.epsilon:
                ...
                tmp = torch.tensor(state).float().detach()
                tmp = tmp.unsqueeze(0)
                qval = self.evaluate_net(tmp)
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        tmp_q = self.target_net.forward(torch.FloatTensor(self.env.reset())).squeeze(0).detach()
        return int(torch.max(tmp_q).numpy())
        # End your code
