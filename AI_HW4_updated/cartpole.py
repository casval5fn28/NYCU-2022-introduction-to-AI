...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        for i in range (len(observation)):
            st.append(self.discretize_value(observation[i], self.bins[i]))
        return tuple(st)
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        """
        # Begin your code
        curr = self.qtable[state + (action, )]
        next_Q = max(self.qtable[next_state])
        self.qtable[state + (action, )] = (1 - self.learning_rate) * curr + self.learning_rate * (reward + self.gamma * next_Q)
        # End your code
        ...
        if done:
            ...
            rwds = []
            for i in range(10):
                st = testing_agent.discretize_observation(testing_agent.env.reset())
                ct = 0
                while True:
                    ct += 1
                    act = np.argmax(testing_agent.qtable[tuple(st)])
                    next_observation, i , done, i = testing_agent.env.step(act)
                    next_st = testing_agent.discretize_observation(next_observation)
                    if done == True:
                        rwds.append(ct)
                        break
                    st = next_st
