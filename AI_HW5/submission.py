...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                post = self.belief.getProb(row, col)
                mean = math.sqrt((agentX - util.colToX(col)) ** 2 + (agentY - util.rowToY(row)) ** 2)
                state = util.pdf(mean, Const.SONAR_STD, observedDist)
                self.belief.setProb(row, col, post * state)
    ...
    def elapseTime(self) -> None:
        ...
        n_belief = util.Belief(self.belief.numRows, self.belief.numCols, value=0)

        for key, value in self.transProb.items():
            (oldTile, newTile) = key
            n_belief.addProb(newTile[0], newTile[1], self.belief.getProb(*oldTile) * value)
            #row,col,delta
        n_belief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for key, value in self.particles.items():
            ...
            mean = math.sqrt((agentX-util.colToX(col))**2 + (agentY-util.rowToY(row))**2)
            state = util.pdf(mean, Const.SONAR_STD, observedDist)
            n_weight[(row, col)] = value * state
        
        for i in range(self.NUM_PARTICLES):
            key = util.weightedRandomChoice(n_weight)
            if key in n_particles:
                n_particles[key] += 1
            else:
                n_particles[key] = 1
    ...
    def elapseTime(self) -> None:
        ...
        n_particles = collections.defaultdict(int)
        for tile in self.particles:
